...
export function ContentCalendar({
  ...
  // Powers the "Filters · N" badge on the trigger button, so collapsing
  // the chip wall into a sheet doesn't cost visibility into what's active.
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-md border border-[var(--border-solid)] px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] transition-colors [transition-duration:var(--transition-fast)] hover:border-[var(--border-hover)] hover:text-foreground active:scale-[0.97]"
        >
          <SlidersHorizontal className="size-3.5" />
          Filters
          {activeFilterCount > 0 && (
            <span className="inline-flex size-4 items-center justify-center rounded-full bg-gold text-[10px] font-semibold text-[var(--accent-foreground)]">
              {activeFilterCount}
            </span>
          )}
        </button>
      ...
      {/* Filters — consolidated from an 11-chip wall into a single sheet.
          Group-level picks (All platforms/pillars/status) use solid gold
          fill when active; individual picks use outline + dot, so the two
          levels of selection read differently at a glance. */}
      <StudioModal
        open={filtersOpen}
      ...
      >
        <div className="space-y-6">
          <div>
            <p className="mb-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-gold/80">
              Platform
            </p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(
                ["all", "draft", "approved", "generated", "coming_soon"] as const
              ).map((s) => (
                <FilterChip
                  key={s}
                  level={s === "all" ? "group" : "item"}
...
/**
 * `level="group"` — the "All X" chip for a category: solid gold fill when
 * active. `level="item"` — an individual pick within that category:
 * outline + colored dot when active, never a solid fill. This keeps group
 * vs. individual selections visually distinct everywhere the app uses
 * filter chips, rather than one flat "selected = gold" rule.
 */
function FilterChip({
  label,
  active,
