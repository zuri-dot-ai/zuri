...
export async function POST(req: Request) {
  ...
  const rateLimit = await checkRateLimit(supabase, user.id, "website:publish");
  ...
  try {
    ...
    if (publishError) {
      const { status, message } = classifySupabaseError(publishError);
    ...
    captureError(err, { supportRef: ref, userId: user?.id, route: "/api/website/publish" });
    return NextResponse.json(
