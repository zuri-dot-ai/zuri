...
| Signup redirects to `*.vercel.app` and restarts onboarding | Set Vercel `NEXT_PUBLIC_APP_URL=https://app.buildzuri.com`. In Supabase Auth → URL Configuration set **Site URL** to `https://app.buildzuri.com` and allow `https://app.buildzuri.com/**`. Callback uses `getAppOrigin()` so redirects never prefer the Vercel host. | — |
